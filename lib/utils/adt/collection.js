 ...
 * /// <reference ref="collection.d.ts" />
 */
import extend from 'extend';
import EventEmitter from 'events';
import { accept, CollectionWithUnderscore, defined, isEqual } from 'utils/utils';
...
class Collection extends EventEmitter {
	...
	_iteratee(elements, predicate) {
		return elements.map(predicate, this).filter(Boolean);
	}
	...
	_matcher(given, element) {
		return isEqual(this._json(given), this._json(element));
	}
	...
	pluckAll(...propertyNames) {
		return this._iteratee(this.elements, (element) => Object.keys(element).reduce((memo, name) =>
